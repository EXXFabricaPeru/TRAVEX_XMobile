...
class <?= Inflector::camel2words(StringHelper::basename($generator->modelClass)) ?> {
    public setting: any;
    public Objet: any;
    ...
    constructor() {
        ...
        this.Objet = Object;
        this.elementGrid = "#<?=Inflector::camel2words(StringHelper::basename($generator->modelClass))?>-list";
    ...
    public windowCreate() {
        ...
        let option = {
            ...
            open: () => {
                this.element.load(this.url + '<?=strtolower(Inflector::camel2words(StringHelper::basename($generator->modelClass)))?>/create');
            },
            buttons: [{
                text: "REGISTRAR",
                class: "btn btn-success",
                click: async () => {
                    let data = $('#<?=Inflector::camel2words(StringHelper::basename($generator->modelClass))?>-form').serialize();
    ...
    public windowEdit(id: any) {
        ...
        let option = {
            ...
            open: () => {
                this.element.load(this.url + '<?=strtolower(Inflector::camel2words(StringHelper::basename($generator->modelClass)))?>/update&id=' + id);
            },
            buttons: [{
                text: "GUARDAR",
                class: "btn btn-success",
    ...
    public windowPdf(id: any) {
        let url = this.url + '<?=strtolower(Inflector::camel2words(StringHelper::basename($generator->modelClass)))?>/report&id=' + id;
        $("#windowpdf").dialog({
    ...
    private requestPut(id: any, data: any) {
        return new Promise((resolve, reject) => {
            $.ajax({
                url: this.url + '<?=strtolower(Inflector::camel2words(StringHelper::basename($generator->modelClass)))?>/update&id=' + id,
                type: 'PUT',
                data: data,
    ...
    private requestDelete(data: any) {
        return new Promise((resolve, reject) => {
            $.get(this.url + '<?=strtolower(Inflector::camel2words(StringHelper::basename($generator->modelClass)))?>/eliminar&id=' + data).done((data) => {
                resolve(data);
            }).fail((err) => {
                reject(err);
            });
    ...
    private requestPost(data: any) {
        return new Promise((resolve, reject) => {
            $.ajax({
                url: this.url + '<?=strtolower(Inflector::camel2words(StringHelper::basename($generator->modelClass)))?>/create',
                type: 'POST',
                data: data,
                success: (data, status, xhr) => {
                    resolve(JSON.parse(data));
...
$(() => {
    let model = new <?= Inflector::camel2words(StringHelper::basename($generator->modelClass)) ?>();
    $("#btn-create").on('click', () => {
        model.windowCreate();
    });
