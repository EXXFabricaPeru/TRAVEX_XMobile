                ...
                try {
                    ...
                    if($val["anulado"] == 1){
                        ...
                        */
                        ...
                        if($val['otpp']==3){
                            ...
                            ."'{$tipocheque}','{$fechaemision}','{$U_LATITUD}','{$U_LONGITUD}','{$emitidoPor}','{$checkdate}','{$transferencedate}','{$tipotarjeta}','{$NumeroTarjeta}','{$NumeroID}','{$val['CreditCard']}','0'";
                            $sql = "SELECT m_insertarPagos($sqlvar) estado";
    
                        }else{
                            if($val['otpp']==2){
                                $facturas = $val["facturas"];
                                foreach ($facturas as $keyf => $factura) {
                                            $sqlfac = "'{$factura['cod']}', '{$factura['recibo']}', '{$factura['pagarx']}','{$factura['clienteId']}','{$factura['docentry']}','{$factura['docnum']}','01','2100','{$factura['nroFactura']}','{$factura['CardName']}','{$factura['saldo']}','{$factura['DocTotal']}'";
                                            $sqlf="SELECT m_f_pagosfacturas($sqlfac) estado";
                                ...
                                . "'{$val['numAutorizacion']}', '{$val['bancoCode']}', '{$val['ci']}', '{$val['fecha']}', '{$val['hora']}', "
                                . " {$val['cambio']}, {$val['monedaDolar']},{$val['otpp']},'{$val['recibo']}',{$val['usuario']},'{$val['cardCreditNumber']}','{$val['baucher']}', '{$centro}', '{$val['DbtCode']}', {$cuota}, '{$equipo}'," 
                                ."'{$tipocheque}','{$fechaemision}','{$U_LATITUD}','{$U_LONGITUD}','{$emitidoPor}','{$checkdate}','{$transferencedate}','{$tipotarjeta}','{$NumeroTarjeta}','{$NumeroID}','{$val['CreditCard']}','{$idCabecera}'";    
                            }else{
                        ...
                        */
                        ...
                        try{
                            ...
                            // CODIGO DE GEDESA//
                            $recibo = Yii::$app->db->createCommand('SELECT * from pagos WHERE id='.$idBaseDatos)->queryOne();
                   ...
                   // $sqlup = "UPDATE numeracion SET numgp = (numgp+1) WHERE iduser = {$val['usuario']};";
                    ...
                    if($val['otpp']==2){
                        ...
                        //$mensaje="error en la creacion de cliente";
                    ...
                    $arr[] = [
                        ...
                        "estado" => 0,// $estadoPago
                        "anulado" => $estadoPagoAnulado,//anulado
                        "control" => $control
                    ];
                }
            //}
