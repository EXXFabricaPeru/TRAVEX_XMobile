...
class ObtenerdocumentosController extends ActiveController {
	...
	public function actionIndex(){
        ...
        */
        ...
        $sql_cabecera="SELECT * FROM historialdocumentos WHERE usuario=".$usuario." and fecha>='".$fechaConsulta."' GROUP BY `idDocumento` ORDER BY `fechaHora` LIMIT 1000";
        $cabecera= Yii::$app->db->createCommand($sql_cabecera)->queryAll();
        ...
        foreach ($cabecera as $key => $value) {
            ...
            ->select('estado')
            ->where("idDocPedido='".$cabecera->cod."' and estado=3")
            ->limit(1)
            ->orderby('id desc')
            ->one();
