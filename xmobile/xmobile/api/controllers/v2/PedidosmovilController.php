...
class PedidosmovilController extends ActiveController {
    ...
    public function actionCreate() {
        ...
        try {
            foreach ($datos as $pedido) {
                ...
                $idHistorial=$this->registraHistorialDoc($pedido);
                if($pedido["header"]["DocType"]=='DFA'){
                    $respuestaPH=Pagos::registrarHistorial($pedido["pagos"]); 
                }
                ...
                if($pedido["header"]["estadosend"] == 7){                
                    ...
                    $id = $this->registerHeader($pedido,$idHistorial);
                    Yii::error("ID: -->".$id);
                    ...
                    if($id!=0){
                        ...
                        Yii::error("Respuesta registro detalle: -->".$respuestaDetalle);
                        //SI EL DETALLE SE REGISTRA CORRECTAMENTE Y SI EL DOCUMENTO ES FACTURA SE REGISTRA EL PAGO
                        Yii::error("Tipo documento -->".$pedido["header"]["DocType"]);
                        if($respuestaDetalle and $pedido["header"]["DocType"]=='DFA' || $pedido["header"]["DocType"]=='DOE'){
                            Yii::error("PAGO DOCUMENTO");                           
                            //SE REGISTRA EL PAGO AL MIDDLEWARE//
                            //PayTermsGrpCode = -1 al contado, todos los numeero deferentes a -1 son a credito
                            if($pedido["header"]["PayTermsGrpCode"]== -1 && $pedido["header"]["DocType"] != 'DOE'){
                                Yii::error("Registra pagos: ".json_encode($pedido["pagos"]));
                                 $respuestaP= Pagos::registrarPago($pedido["pagos"],$id,$respuestaPH['id']);// Registro middleware  
                            }
                            else{
                                $respuestaP = [
                                    "id" => 0,
                            ...
                            yii::error("respuesta a credito".$respuestaP['registro']);
                            if($respuestaP['registro']){
                        
                                Yii::error("se registro el pago a midd");
                                //ENVIO FACTURA SAP//
                                if($pedido["header"]["DocType"]!='DOE')
                                    $respuestaEnvio=Sapenviodoc::facturas($id);
                                else
                                    $respuestaEnvio=Sapenviodoc::entrega($id);

                                //ENVIO PAGO A SAP//
                                  yii::error("data response sap->".json_encode($respuestaEnvio));
                                  //and $respuestaP['id']!=0
                                if($respuestaEnvio['registro'] && $respuestaP['registro'] ){
                                    if($respuestaEnvio['estado'] && $respuestaEnvio['estado']==3 ){
                                        $this->updatenumeracion($pedido["usuariodataid"], $pedido["header"]["DocType"]);
                                        yii::error("actualizando numeracion");
                                ...
                                $respuestaEnvio = [
                                    ...
                                    "numeracion"=>0,
                                    "mensaje"=>"EL documento no paso a sap - revisar pago"
                                ];
                            }   
                        }elseif($respuestaDetalle and $pedido["header"]["DocType"]!='DFA'){
                            //ENVIO DEL DOCUMENTO QUE NO ES FACTURA A SAP//
                           
                            if($pedido["header"]["DocType"]=="DOF" OR $pedido["header"]["DocType"]=="DOP"){
                ...
                if($pedido["header"]["DocType"]=='DFA'){
                    ...
                    if($arr['estadoDoc']==3 and $arr['estadoPago']!=3){
                        $arr['mensajeDoc']="Factura enviada a Sap. Pago rechazado, vuelva a registrar el pago";//, mensaje sap: ".$arr['mensajePago'];
                    }
                }else{
                    $arr = [
                        "estadoDoc" =>$respuestaEnvio['estado'],
    ...
    public function registerDetalles($id, $detalles, $clone, $pedido,$listaPrecio) {
        if (count($detalles) > 0) {
            ...
            $sumadetalle=0;// nueva linea mau para sumar detalles y ponerlos como docTotalPay
            foreach ($detalles as $lineaPedido) {
                if ($lineaPedido["LineTotal"] >= 0) {
                    ...
                    //nuevos campos de descuento en el detalle
                    $linea->DiscTotalPrcntCab = $lineaPedido['XMPORCENTAJEBONIFICACION'];
                    $linea->DiscTotalMonetaryCab = $lineaPedido['XMVALORBONIFICACION'];
                    ...
                    /* lineas para sumar detalle y ponerlo como doctotalPay
                    $aux_totalLinea=0;
                    $aux_totalLinea=($lineaPedido["Quantity"]*$lineaPedido["Price"])+$lineaPedido['ICEe']+$lineaPedido['ICEp']-$lineaPedido["U_4DESCUENTO"];
                    $sumadetalle=$sumadetalle+$aux_totalLinea;
                     Fin lineas para sumar detalle y ponerlo como doctotalPay
                    */
				}
            }
            /* lineas para update doctotalPay en cabecera documentos
            $aux_sql_updateCabecera = ' UPDATE cabeceradocumentos SET DocTotalPay2 = "'.$sumadetalle.'" WHERE id = ' . $id;
            $db->createCommand($aux_sql_updateCabecera)->execute();
            Fin lineas para update doctotalPay en cabecera documentos
            */ 
            return true;
        }
        else{
    ...
    public function registerHeader($pedido,$idHistorial) {
        Yii::error('documeto cabecera --------------->'. json_encode($pedido["header"]));
        ...
        $cabecera->monto=$pedido["header"]["U_MontoCampania"];
        $cabecera->fex_tipodoc=$pedido["header"]["Fex_tipodocumento"];
        $cabecera->idHistorial=$idHistorial;
        //$cabecera->campania = $pedido["header"]["U_CodigoCampania"];
        //$cabecera->monto = $pedido["header"]["U_ValorSaldo"];
        try{
            $transportista = $pedido["header"]["transportista"];
            $cabecera->U_EXX_CODTRANS = $transportista["CardCode"];
            $cabecera->U_EXX_NOMTRANS = $transportista["CardName"];
            $cabecera->U_EXX_RUCTRANS = $transportista["LicTradNum"];
            $cabecera->U_EXX_DIRTRANS = $transportista["Address"];
            $cabecera->U_EXX_NOMCONDU = $transportista["Name"];
            $cabecera->U_EXX_LICCONDU = $transportista["Notes1"];
            $cabecera->U_EXX_PLACAVEH = $transportista["U_EXX_PLAVEH"];
            $cabecera->U_EXX_MARCAVEH = $transportista["U_EXX_MARVEH"];
            $cabecera->U_EXX_PLACATOL = $transportista["U_EXX_PLATOL"];
            if ($transportista["flagTipoDoc"]){
                $query_series = "SELECT Series FROM series WHERE Locked='N' AND IsElectronicCommEnabled='Y'";
                $res_series=Yii::$app->db->createCommand($query_series)->queryOne();
                $cabecera->Series = $res_series["Series"];
            }
    ...
    private function validaExisteDocumento($pedido){
        ...
        /* solo para probar el error de numero duplicado
        if($pedido["header"]["idUser"]==5){
