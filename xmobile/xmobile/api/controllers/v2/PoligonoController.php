...
class PoligonoController extends ActiveController {
    ...
    public function actionIndex() {
        ...
        $idUsuario = Yii::$app->request->post('idvendedor');
        ...
        Yii::error("Datos P :".json_encode($usuarioConfig));
		if($usuarioConfig['TipoUsuario']==0){//por verdad SI ES VENDEDOR Vendedor
            ...
            foreach ($resultado as $key => $value) {
				...
				$rutadetalle[$key]['latitud'] = $this->convertCoordenadas($value['U_Latitud']);
				$rutadetalle[$key]['longitud'] = $this->convertCoordenadas($value['U_Longitud']);
				$rutadetalle[$key]['territoryid'] = $value['U_Territorio'];
				$rutadetalle[$key]['territoryname'] = '';
				$rutadetalle[$key]['posicion'] = ($key+1);
    ...
    private function convertCoordenadas($coordenada){
        if($coordenada!=null){   
