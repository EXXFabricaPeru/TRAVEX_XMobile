...
protected function verbs()
  ...
  public function actionCreate(){
    ...
    Yii::error("INGRESO ANULAR PAGO".json_encode($data));

    //se valida si el usuario tiene permiso para anular pagos 
    $sql_v = "SELECT anularcobros FROM usuarioconfiguracion WHERE idUser =".$data['usuario'];
    $dataV = Yii::$app->db->createCommand($sql_v)->queryOne();
    if($dataV['anularcobros']==0){
      return ["estado"=>1,"mensaje"=>"No tiene permiso para anular pagos."];
    }
    //fin de validacion//
    ...
    Yii::error("Respuesta envio pago: ".json_encode($dataEnvio));
    return $dataEnvio;
