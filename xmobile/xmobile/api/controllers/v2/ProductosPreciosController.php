...
class ProductospreciosController extends ActiveController {
    ...
    public function actionIndex(){
        ...
        $modelo= $modelo->obtenerProductosPrecios($usuario,$salto);
        if (count($modelo)){
    ...
    public function actionContador(){
        ...
        $usuario=Yii::$app->request->post('usuario');        
        $modelo=new Productos;
        $modelo= $modelo->obtenerProductosPreciosContador($usuario);
        return $this->correcto($modelo, 'OK'); 
    }  
    public function actionBuscador(){
        $equipo=Yii::$app->request->post('equipo');
        $usuario=Yii::$app->request->post('usuario');
        $lista=Yii::$app->request->post('lista'); 
        $texto=Yii::$app->request->post('texto');       
        $modelo=new Productos;
        $modelo= $modelo->obtenerProductosPreciosTodos($usuario,$texto,$lista);
        return $this->correcto($modelo, 'OK'); 
    }
}