...
class PoligonocabeceraterritorioController extends Controller
{
        ...
        return [
            ...
            'access' => [
                ...
                'rules' => [
                    [
                        'allow' => true,
                        'actions' => ['index', 'create', 'update', 'view', 'eliminar', 'report','usuarioterritorio','poligonocabecera','poligonodetalle','listacliente','guardardetalle','listaclienteguardados','updatedetalle','listaiconos','verifcaasignacion','view_poligono','ver','listaclienterutas','listaclienterutasdespacho','listaterritoriocliente'],
                        'roles' => ['@']
                    ]
                ],
            ],
    ...
    public function actionVer($id=0) {
        $datos = Yii::$app->request->post();
    ...
    public function actionListaclienterutas() {
        Yii::error("Entrada a texto CLIENTES RUTAS");
        $datos = Yii::$app->request->post();
        Yii::error($datos['vendedor']);
        //$usuariomovilterritorio = Usuariomovilterritorio::find();
        $DataPoligonocliente = Poligonocliente::find()->where('vendedor = '.$datos['vendedor'].' and dia='.$datos['dia'])->orderby('posicion asc')->asArray()->all();
        Yii::error($DataPoligonocliente);
        return json_encode($DataPoligonocliente);
       
    }

    public function actionListaclienterutasdespacho() {
        Yii::error("Entrada a texto CLIENTES RUTAS");
        $datos = Yii::$app->request->post();
        Yii::error($datos['vendedor']);
        $clientes = new Documentos();
        $result= $clientes->obtenerListaPickingFecha($datos['fecha'],$datos['vendedor']);
        foreach ($result as $key => $value) {
            $result[$key]['latitud']=$this->convertCoordenadas($value['latitud']);
            $result[$key]['longitud']=$this->convertCoordenadas($value['longitud']);
        }
        Yii::error($result);
        return json_encode($result);  
    }

    public function actionListaterritoriocliente() {
        Yii::error("Entrada a texto CLIENTES TERRITORIOS");
        $datos = Yii::$app->request->post();
        Yii::error($datos['territorioid']);
        $clientes = new Clientes();
        $campos='C."CardCode" as "cardcode",C."CardName" as "cardname",C."U_XM_Latitud" as "latitud", C."U_XM_Longitud" as "longitud", C."Territory" as "territorio", C."Address" as "calle", "OTER"."descript" as "territoryname"';
        $condicion=" LEFT JOIN \"OTER\" ON C.\"Territory\"=\"OTER\".\"territryID\"
        WHERE C.\"Territory\"=".$datos['territorioid'];
        $result= $clientes->obtenerCamposEspecificosLista($campos,$condicion);
        foreach ($result as $key => $value) {
            $result[$key]['latitud']=$this->convertCoordenadas($value['latitud']);
            $result[$key]['longitud']=$this->convertCoordenadas($value['longitud']);
        }
        ...
        //verificando si la tatitud y longitus estan bien//
        $poligonodetalle = Poligonodetalle::find()->where("idCabecera=1")->asArray()->all(); 
        $poligono = [];
        $Dataclientes=[];
        $DataclientesSinCoor=[];
        // codigo para controlar si el cliente esta dentro del mapa
        foreach ($poligonodetalle as $key => $value) {
            array_push($poligono, $value["latitud"].' '.$value["longitud"] );
        }
        $pointLocation = new pointLocation();
        foreach ($result as $key => $valorCliente) {
            ...
            Yii::error($respuesta);
            if ($respuesta == 'inside') {            
                array_push($Dataclientes, $valorCliente);
            }
            else{
                 array_push($DataclientesSinCoor, $valorCliente);
            }
        }

        return (json_encode (["clientescon"=>$Dataclientes,"clientessin"=>$DataclientesSinCoor]));
    ...
    public function actionListacliente() {
        ...
        foreach ($result as $key => $valorCliente) {
            ...
            if($valorCliente["latitud"]!=null){
                /*$latitud=$valorCliente["latitud"];
                $longitud=$valorCliente["longitud"];                
                $tamanio=strlen($latitud);
                $tamanio2=strlen($longitud);
                Yii::error("tamanio :".$tamanio);
                $latitud1=substr($latitud,0,4);
                $longitud1=substr($longitud,0,4);
                Yii::error("latitud1 :"+$latitud1);
                $latitud2=substr($latitud,4,$tamanio);
                $longitud2=substr($longitud,4,$tamanio2);
                Yii::error("latitud2 :".$latitud2);
                $latitud2=str_replace('.', '',$latitud2);
                $longitud2= str_replace('.', '',$longitud2);
                Yii::error("latitud2 replaze :".$latitud2);
                $latitud=$latitud1.''.$latitud2;
                $longitud=$longitud1.''.$longitud2;
                Yii::error("latitud :".$latitud);
                Yii::error("longitud :".$longitud);
                $valorCliente["latitud"]=$latitud;
                $valorCliente["longitud"]=$longitud; */
                $latitud=$valorCliente["latitud"];
                $longitud=$valorCliente["longitud"];                
                $tamanio=strlen($latitud);
                $tamanio2=strlen($longitud);
                ...
                $latitud1=substr($latitud,0,3);
                $longitud1=substr($longitud,0,3);
                Yii::error("latitud1 :"+$latitud1);
                $latitud2=substr($latitud,3,$tamanio);
                $longitud2=substr($longitud,3,$tamanio2);
                Yii::error("latitud2 :".$latitud2);
        ...
        // FIN codigo para controlar si el cliente esta dentro del mapa
   ...
   // ));
    ...
    function convertCoordenadas($coordenada){
        if($coordenada!=null){   
            $tamanio=strlen($coordenada);

            Yii::error("tamanio :".$tamanio);
           
            $coordenada=str_replace('.', '',$coordenada);

            $coordenada1=substr($coordenada,0,3);
            Yii::error("coordenada1 :"+$coordenada1);
            $coordenada2=substr($coordenada,3,$tamanio);
            Yii::error("coordenada2 :".$coordenada2);
            $coordenada=$coordenada1.'.'.$coordenada2;
            Yii::error("coordenada :".$coordenada);
        }
        return $coordenada;
    }
