...
class Productos extends Model {
    ...
    public function obtenerTodosProductos($equipo,$usuario,$almacen,$texto){
        $sql=" Select ";
        $campos=$this->obtenerCamposConsultaProductos();        
        $from=' from "OITM" P LEFT JOIN  "OITB" PG ON P."ItmsGrpCod"=PG."ItmsGrpCod"';
        
        $where=$this->obtenerCondicionConsulta(0,$usuario,$texto);
    ...
    public function obtenerProductos($equipo,$usuario,$salto=0,$limite=1000){
        ...
        Yii::error(json_encode("Consulta Productos:"));
        Yii::error(json_encode($sql_hana));
        $resultado=  $this->hana->ejecutarconsultaAll($sql_hana);
    ...
    private function obtenerCondicionConsulta($equipo=0,$usuario,$texto=''){
        ...
        // $zonaFranca=$configuracion_tipo_vendedor["zonaFranca"];
        ...
        /*if($zonaFranca==1){
            $cadena.=' and P."U_XM_ICEtipo"=\'N\' and P."U_XM_ICEPorcentual"=\'0\' and P."U_XM_ICEEspecifico"=\'0\'';
        }*/
        return $cadena;
    ...
    private function obtenerCondicionConsultaProductosAlmacenes($equipo,$almacen='',$texto=''){
        ...
        if($almacen != ''){
            $cadena=' where PA."WhsCode"=\''.$almacen.'\' ';
             ...
             $cadena=" where PA.\"WhsCode\" in('PTVES')";
        }
        if($texto!=''){
    ...
    private function obtenerCamposConsultaProductosPrecios(){
        ...
        $sql='
            ...
            PP."AddPrice2",
            PP."Currency2",
            -1 as "UgpEntry", -- PP."UgpEntry",
            0 as "Code", --PP."UomCode" as "Code",
            \'\' as "Name",-- PP."UomName" as "Name",
            0 as "BaseQty",-- PP."BaseQty",
            PP."PriceList" as "PriceListNo" ,         
            \'\' as "PriceListName" --PP."ListName" as "PriceListName"
        ';
        return $sql;
    }
...
// funciones Productos Series
    ...
    public function obtenerCamposEspecificosLotesTodos($campos,$where){        
        $sql=" Select ";       
        $from=" from \"EXX_XM_LotesProductos\" ";
        $sql_hana=$sql." ". $campos." ".$from." ".$where;
        Yii::error("lotes consulta : ".$sql_hana);
        $resultado=  $this->hana->ejecutarconsultaAll($sql_hana);
        return  $resultado;
    }
    ...
    public function Lista_productos_stock($codigo,$almacen,$cantidad) {
        try {
            $sql='Select Count(*) as cantidad ';
            $from=' from "OITW" ';
            $where="  WHERE \"ItemCode\" = '".$codigo."' AND \"WhsCode\" = '".$almacen."' AND (\"OnHand\"-\"IsCommited\") >= '".$cantidad."'";  
            $sql_hana=$sql." ".$from." ".$where;
            $resultado=  $this->hana->ejecutarconsultaAll($sql_hana);
            return $resultado;
        } catch (\Throwable $e) {
