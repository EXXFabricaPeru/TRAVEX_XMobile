...
var User = (function () {
    function User() {
        ...
        this.Objet = Object;
        this.elementGrid = "#User-list";
        this.setting = { width: 450, height: 450, hide: 'fade', show: 'fade', modal: true };
    }
    User.prototype.windowCreate = function () {
        var _this = this;
        this.element.html('<div class="loader">Loading...</div>');
        var option = {
            title: 'NUEVO REGISTRO',
            open: function () {
                _this.element.load(_this.url + 'user/create');
            },
            buttons: [{
                    text: "REGISTRAR",
    ...
    User.prototype.windowEliminar = function (id) {
        var _this = this;
        var obj = {
    ...
    User.prototype.windowEdit = function (id) {
        var _this = this;
        this.element.html('<div class="loader">Loading...</div>');
        var option = {
            title: 'ACTUALIZAR REGISTRO',
            open: function () {
                _this.element.load(_this.url + 'user/update&id=' + id);
            },
            buttons: [{
                    text: "GUARDAR",
                    class: "btn btn-success",
    ...
    User.prototype.windowPdf = function (id) {
        var url = this.url + 'user/report&id=' + id;
        $("#windowpdf").dialog({
    ...
    User.prototype.requestPut = function (id, data) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            $.ajax({
                url: _this.url + 'user/update&id=' + id,
                type: 'PUT',
                data: data,
    ...
    User.prototype.requestDelete = function (data) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            $.get(_this.url + 'user/eliminar&id=' + data).done(function (data) {
                resolve(data);
            }).fail(function (err) {
                reject(err);
            });
    ...
    User.prototype.requestPost = function (data) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            $.ajax({
                url: _this.url + 'user/create',
                type: 'POST',
                data: data,
                success: function (data, status, xhr) {
                    resolve(JSON.parse(data));
...
$(function () {
	...
	/***********************/
	$(document).on("click", ".resetEventData", function () {
        var id = $(this).attr('id');
		        var obj = {
            width: 350, height: 200, hide: 'fade', show: 'fade', modal: true, title: 'ALERTA',
            buttons: [{
                    ...
                    click: function () { 
						    $.ajax({
								url: $("#PATH").attr("name") + 'resetx/update&id=' + id,
								type: 'PUT',
								data: {"pass": ""},
								success: function (data, status, xhr) {
									  $("#windowReset").dialog("close");
									$.pjax.reload({ container: "#User-list", async: false });
								},
								error: function (jqXhr, textStatus, errorMessage) {
									alert('Verifica tu conexion');
								}
							});
					}
                }, {
