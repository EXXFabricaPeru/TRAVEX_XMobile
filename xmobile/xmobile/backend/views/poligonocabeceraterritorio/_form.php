...
<style>
    ...
    #hdClientessin td, #hdClientessin th {
       border: 1px solid #ddd;
        padding: 5px;
    }

    #hdClientessin tr:nth-child(even){background-color: #f2f2f2;}

    #hdClientessin tr:hover {background-color: #ddd;}
...
<div class="container-fluid">
        ...
        <div class="col-md-6" style="height:500px;overflow:auto;">  
            ...
            <div class="row">
                ...
                <div class="col-md-6" >  
                    ...
                    <div class="row">
                        <div class="col-md-12" > 
                            <button type="button" class="btn btn-success form-control" onclick="ListarClientes()">Listar Clientes</button>
                        </div>
                        <!--div class="col-md-3" > 
   ...
   // if($model->idVendedor==''){
    ?>
    <div class="row" style="display: none;" id="div-clientes-sincoor">
        <hr>
        <div class="col-md-6">  
            <h4>
                <div class="row">
                    <div class="col-lg-4">Clientes Con Coordenas Incorrectas: <span id="cantidadClientesSin"> </span></div>
                    <div class="col-lg-2">Buscador:</div>
                    <div class="col-lg-6">
                    <input id="searchTermSin" type="text" class="form-control" onkeyup="doSearchSin()" />
                    </div>
                </div>
            </h4>
           <div class="" style="height:300px;overflow:auto;">
                <table id="hdClientessin">
                    <thead>
                        <th>Codigo</th>
                        <th>Nombre</th>                      
                        <th>Latitud</th>
                    ...
                    <tbody id="tableDetalleSin">
                    </tbody>
                </table>
                <div class="well">
                    <p id="p-resultado-sin"> </p>  
                </div>
            </div>
        </div>
        <div class="col-md-6">
...
<script>
    ...
    $('document').ready(function () {
        //cargarDatos();
        //initMap(0);
        map = L.map('map').setView([-16.496777, -68.132031], 13);

        L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
        maxZoom: 19,
        attribution: '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>'
        }).addTo(map);
        //inicio de un nuevo api de mapas
    ...
    function placeMarker(location, color, informacion = null) {
        ...
        allMarkers.push(marker);//para calcular ruta
  
    }

    function placeMarkerApi(location, color, informacion = null) {
        console.log("Localizacion RNP: "+location.lat+" "+location.lng);

        var marker = L.marker([Number(location.lat),Number(location.lng)]).addTo(map);
        var info = '';
        if (informacion != null){
            info = "Codigo: " + informacion.CardCode + '<br>' + 'Nombre: ' + informacion.CardName+'<br>'+informacion.Direccion;
        }
...
//////////////////////////////////////////////////////////
        ...
        if(idTerritorio!=""){
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    console.log(data);*/
                    ArrayDataTodos = $.parseJSON(data);
                    console.log("POLIGONO CLIENTE");
                    console.log(ArrayDataTodos);
                    ArrayData=ArrayDataTodos.clientescon;
                    console.log(ArrayData);
                    ArrayDataSin=ArrayDataTodos.clientessin;
                    console.log(ArrayDataSin);                   
                    var i=0;
                    var contenido="";
                        ...
                        for(i;i<ArrayData.length;i++){
                            ...
                            */
                            ...
                            if(i<2000){
                                ...
                                contenido=contenido+ AdicionarRow((i+1),ArrayData[i].CardCode,cardName,latitud,longitud,direccion,'tableDetalle',ArrayData[i].territorio,ArrayData[i].idPoligono,i+1,swRuta,territorio);
                    ...
                    //cargarPoligono();
                    //funcionalidad del nuevo api
                    asignaEtiquetas(1);

                    //clientes sin coordenadas correctas
                    var i=0;
                    var contenidoSin="";
                    for(i;i<ArrayDataSin.length;i++){  
                        var cardName=ArrayDataSin[i].CardName;
                        var tamanioCardName=cardName.length;
                        ...
                        var latitud=ArrayDataSin[i].latitud;
                        var longitud=ArrayDataSin[i].longitud;                        
                        contenidoSin=contenidoSin+ AdicionarRowSin(ArrayDataSin[i].CardCode,cardName,latitud,longitud);   
                    }
                    if(ArrayDataSin.length>0){
                       $("#div-clientes-sincoor").show();
                       var items = document.getElementById('tableDetalleSin');
                       console.log("contenidoSin");
                       //console.log(contenidoSin);
                       items.innerHTML = contenidoSin;
                    }
                    else{
                        $("#div-clientes-sincoor").hide();   
                    }

                    $("#cantidadClientesSin").text(i);
    ...
    //alert(tabla);
      //  var items = document.getElementById(tabla);
        ...
        var contenido = '<tr  id="tr-fila-'+idAux+'" title="'+swObservacion+'"  style="'+stylo+'"  onclick="mostrarEtiquetaApi('+idAux+')" >  '+
    ...
    function AdicionarRowSin(cardcode,cardname,latitud,longitud){

        
        var contenido = '<tr>'+
                    '<td style="" >' + cardcode + '</td>'+//latitude
                    '<td style="">' + cardname + '</td>'+ //longitude   
                    '<td style="">' + latitud + '</td>'+ //longitude   
   ...
   function EliminarFila(id){
    ...
    function mostrarEtiquetaApi(id){
        var Row = document.getElementById("tr-fila-"+id);
        cambiarColor(Row);
        var cardCode= $(Row).find('td').eq(1).html(); 
        var cardName= $(Row).find('td').eq(2).html(); 
        ...
        */
        ...
        var marker = L.marker([Number(latitud),Number(longitud)]).addTo(map);

        info = "Codigo: " + cardCode + '<br>' + 'Nombre: ' + cardName+'<br>'+calle;
            ...
            if(radioCheked){
               // if($('#tableDetalle').children().length<=90){
                    ...
                    // var xordenado = clone(waypts);
                        ...
                        //alert("cantidad:"+stations.length);
    ...
    function calcularDistancia(p1, p2) {//verificar funcionalidad
    ...
    function asignaEtiquetas(color){
        ...
        $('#tableDetalle tr').each(function () {
            ...
            if (esVisible){
                ...
                placeMarkerApi(coor, color, { CardCode:  cod, CardName:  nom, Direccion:  direc });
            } 
        });
    }

    ////
    function elimanarMarker(coordenadas){
        for (var i = 0; i < allMarkers.length; i++) {
