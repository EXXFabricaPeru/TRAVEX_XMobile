...
export class Bonificacion_ca extends Databaseconf {
    ...
    public async getBonificacionesDisponibles(grupoCliente, grupoProducto) {
        let dateNow = moment().format('YYYY-MM-DD');
        // let fecha = moment().format('YYYY-MM-DD h:mm:ss');
        // console.log("dateNow ", dateNow);
        // let hora = moment().format('h:mm:ss');
        let sqlgrupoCliente = "";
        let sqlgrupoProducto = "";
        if (grupoCliente != 0 || grupoCliente != "0") {
            sqlgrupoCliente = " AND grupo_cliente='" + grupoCliente + "'";

        } else {
            sqlgrupoCliente = " AND grupo_cliente='0'";
        }
        if (grupoProducto != "PRODUCTOS ESPECIFICOS") {

            sqlgrupoProducto = " AND tipo='" + grupoProducto + "'";
        } else {
            sqlgrupoProducto = " AND tipo='" + grupoProducto + "'";
        }
        console.log("DEVD for sql grupoCliente ", grupoCliente, " grupoProducto ", grupoProducto);//${sqlgrupoProducto}  ${sqlgrupoCliente}
        let sql = `SELECT * FROM bonificacion_ca WHERE DATE() BETWEEN DATE(fecha_inicio) AND DATE(fecha_fin)  ORDER BY id DESC LIMIT 1 ;`;// ORDER BY id DESC LIMIT 1 
        console.log("sql ", sql);
        return await this.queryAll(sql);
    }
